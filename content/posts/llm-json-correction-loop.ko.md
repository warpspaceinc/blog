...
작은 fixture (16 entity / 14 edge, 6 결함, 6 seeds 평균):
...
## path_finder + narrowing이 gap을 닫는다
...
## 가장 의외였던 발견: narrowing은 cost가 아니라 correctness다
...
1. **Full-regen은 우리가 생각하는 것보다 일찍 깨진다.** 100 entity는 production-realistic 사이즈고, gpt-4o-mini로는 동작하지 않습니다.
...
라이브러리는 [github.com/warpspaceinc/json-correction](https://github.com/warpspaceinc/json-correction)에 OSS로 공개돼 있습니다. Apache-2.0, `pip install` 가능 (PyPI 곧 등록 예정), 작은 quickstart 예제부터 KG 벤치마크까지 포함. 본인 도메인 critic만 plug-in 하면 됩니다.

큰 JSON 다루는 LLM 시스템 운영하시는 분들, 한 번 측정해보세요. 본인 시스템이 *언제* 깨지기 시작하는지 모르고 있을 가능성이 높습니다.
